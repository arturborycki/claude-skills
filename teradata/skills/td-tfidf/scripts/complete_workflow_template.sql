-- Complete TD_TFIDF Workflow Template
...
-- 3. TD_TFIDF Execution
DROP TABLE IF EXISTS td_tfidf_results;
CREATE MULTISET TABLE td_tfidf_results AS (
    SELECT * FROM TD_TFIDF (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_TFIDF parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_tfidf_results ORDER BY 1;
