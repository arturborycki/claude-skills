-- =====================================================
-- TD_MovingAverageForecaster - UAF Data Preparation
-- =====================================================
-- Purpose: Prepare time series data for UAF moving average forecasting
-- Function: TD_MovingAverageForecaster
...
DROP TABLE IF EXISTS {USER_DATABASE}.uaf_movavg_forecast_input;
CREATE MULTISET TABLE {USER_DATABASE}.uaf_movavg_forecast_input AS (
    SELECT
        {TIMESTAMP_COLUMN} as time_index,
...
SELECT
    'UAF TD_MovingAverageForecaster Readiness' as status_type,
    COUNT(*) as n_observations,
    CASE
    ...
    CASE
        WHEN COUNT(*) >= 30 THEN 'READY FOR TD_MovingAverageForecaster'
