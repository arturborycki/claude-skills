-- Complete TD_MOVAVG_FORECAST UAF Workflow Template
...
-- 2. TD_MOVAVG_FORECAST Execution
DROP TABLE IF EXISTS td_movavg_forecast_results;
CREATE MULTISET TABLE td_movavg_forecast_results AS (
    SELECT * FROM TD_MOVAVG_FORECAST (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- WindowSize (configure based on your analysis requirements)
        -- ForecastPeriods (configure based on your analysis requirements)
        -- WeightingScheme (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_MOVAVG_FORECAST parameters
    ) as dt
) WITH DATA;
...
FROM td_movavg_forecast_results;

-- 4. Results Export (if needed)
SELECT * FROM td_movavg_forecast_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_movavg_forecast_results;
