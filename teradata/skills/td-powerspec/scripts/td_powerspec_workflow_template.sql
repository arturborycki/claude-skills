-- Complete TD_PowerSpec UAF Workflow Template
...
-- 2. TD_PowerSpec Execution
DROP TABLE IF EXISTS td_powerspec_results;
CREATE MULTISET TABLE td_powerspec_results AS (
    SELECT * FROM TD_PowerSpec (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- WindowType (configure based on your analysis requirements)
        -- SegmentSize (configure based on your analysis requirements)
        -- OverlapRatio (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_PowerSpec parameters
    ) as dt
) WITH DATA;
...
FROM td_powerspec_results;

-- 4. Results Export (if needed)
SELECT * FROM td_powerspec_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_powerspec_results;
