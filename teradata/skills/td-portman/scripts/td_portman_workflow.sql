-- Complete TD_PORTMAN UAF Workflow Template
...
-- 2. TD_PORTMAN Execution
DROP TABLE IF EXISTS td_portman_results;
CREATE MULTISET TABLE td_portman_results AS (
    SELECT * FROM TD_PORTMAN (
        ...
        -- Refer to Teradata UAF documentation for TD_PORTMAN parameters
    ) as dt
) WITH DATA;
...
FROM td_portman_results;

-- 4. Results Export (if needed)
SELECT * FROM td_portman_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_portman_results;
