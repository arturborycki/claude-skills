-- =====================================================
-- TD_SeasonalDecompose - UAF Performance Monitoring
-- =====================================================
-- Purpose: Monitor and optimize TD_SeasonalDecompose execution
-- Function: TD_SeasonalDecompose performance tracking
...
SELECT
    ...
    'TD_SeasonalDecompose',
    '{USER_DATABASE}',
    'uaf_seasonal_decompose_input',
    CURRENT_TIMESTAMP,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input),
    'RUNNING';
...
WHERE DatabaseName = '{USER_DATABASE}'
  AND TableName IN ('uaf_seasonal_decompose_input', 'seasonal_decompose_results')
ORDER BY TableName;
...
FROM {USER_DATABASE}.uaf_seasonal_decompose_input
GROUP BY HashAmp()
ORDER BY amp_number;
...
-- Monitor recent TD_SeasonalDecompose executions
SELECT
    QueryID,
    UserName,
    StartTime,
    ...
    NumResultRows
FROM DBC.QryLog
WHERE QueryText LIKE '%TD_SeasonalDecompose%'
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) as n_input_observations,
    (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) as n_output_results,
    (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) as n_valid_results,
    CAST(
        (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) /
        CAST((SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) AS FLOAT) * 100
        AS DECIMAL(8,2)
    ) as completeness_pct,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) AS FLOAT) >= 0.95
        THEN 'Excellent - High completeness (>=95%)'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) AS FLOAT) >= 0.80
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) as total_result_rows,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'seasonal_decompose_results') as total_size_mb,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'seasonal_decompose_results') / NULLIF((SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results), 0) as avg_mb_per_1000_rows;

-- Identify optimization opportunities
SELECT
    'Optimization Recommendations' as recommendation_type,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) > 10000
    ...
    CASE
        WHEN (SELECT CurrentPerm / 1024 / 1024
              FROM DBC.TableSize
              WHERE DatabaseName = '{USER_DATABASE}' AND TableName = 'seasonal_decompose_results') > 1000
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input WHERE series_value IS NULL) as null_values,
    (SELECT COUNT(*)
     FROM {USER_DATABASE}.uaf_seasonal_decompose_input
     WHERE ABS(series_value - (SELECT AVG(series_value) FROM {USER_DATABASE}.uaf_seasonal_decompose_input)) >
           3 * (SELECT STDDEV(series_value) FROM {USER_DATABASE}.uaf_seasonal_decompose_input)) as outliers_3sd,
    (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) as distinct_timestamps,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) as total_rows,
    CASE
        WHEN (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) =
             (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input)
...
WHERE function_name = 'TD_SeasonalDecompose'
...
-- Overall UAF TD_SeasonalDecompose health metrics
SELECT
...
SELECT
    ...
    CASE
        WHEN (SELECT SUM(CurrentPerm) / 1024 / 1024
              FROM DBC.TableSize
              WHERE DatabaseName = '{USER_DATABASE}'
                AND TableName LIKE '%seasonal_decompose%') > 5000
