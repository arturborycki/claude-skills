-- =====================================================
-- TD_SeasonalDecompose - Result Interpretation
-- =====================================================
-- Purpose: Interpret and visualize seasonal decomposition results
-- Function: TD_SeasonalDecompose diagnostic analysis
-- Framework: Teradata Unbounded Array Framework (UAF)
-- =====================================================

-- PREREQUISITE: Run td_seasonal_decompose_workflow.sql first
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) as input_observations,
    (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) as result_observations,
    CAST((SELECT AVG(series_value) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) AS DECIMAL(12,4)) as input_avg,
    CAST((SELECT AVG(result_value) FROM {USER_DATABASE}.seasonal_decompose_results) AS DECIMAL(12,4)) as result_avg,
    CAST((SELECT STDDEV(series_value) FROM {USER_DATABASE}.uaf_seasonal_decompose_input) AS DECIMAL(12,4)) as input_std,
    CAST((SELECT STDDEV(result_value) FROM {USER_DATABASE}.seasonal_decompose_results) AS DECIMAL(12,4)) as result_std;
...
SELECT
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) * 0.95
        THEN 'High quality results - suitable for business decision making'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) * 0.80
...
SELECT
    ...
    'Recommended Actions',
    CASE
        WHEN (SELECT STDDEV(result_value) FROM {USER_DATABASE}.seasonal_decompose_results) /
             NULLIF((SELECT AVG(result_value) FROM {USER_DATABASE}.seasonal_decompose_results), 0) < 0.2
        THEN 'Results show low variability - patterns are consistent'
        WHEN (SELECT STDDEV(result_value) FROM {USER_DATABASE}.seasonal_decompose_results) /
             NULLIF((SELECT AVG(result_value) FROM {USER_DATABASE}.seasonal_decompose_results), 0) < 0.5
...
SELECT
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NULL) = 0
...
SELECT
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.seasonal_decompose_results) * 0.90
