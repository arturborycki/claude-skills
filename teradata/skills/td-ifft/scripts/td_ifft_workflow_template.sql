-- Complete TD_IFFT UAF Workflow Template
...
-- 2. TD_IFFT Execution
DROP TABLE IF EXISTS td_ifft_results;
CREATE MULTISET TABLE td_ifft_results AS (
    SELECT * FROM TD_IFFT (
        ...
        -- Refer to Teradata UAF documentation for TD_IFFT parameters
    ) as dt
) WITH DATA;
...
SELECT * FROM td_ifft_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_ifft_results;
