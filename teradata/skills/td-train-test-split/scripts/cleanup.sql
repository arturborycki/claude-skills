...
DROP TABLE IF EXISTS {database}.train_data_out;
DROP TABLE IF EXISTS {database}.test_data_out;
DROP TABLE IF EXISTS {database}.validation_data_out;
*/

-- Option 2: Selective cleanup - Keep train and test sets
/*
-- Keep: train_data_out, test_data_out
DROP TABLE IF EXISTS {database}.train_test_out;
DROP TABLE IF EXISTS {database}.validation_data_out;
*/
...
/*
CREATE TABLE {database}.train_data_archive AS
    SELECT *, CURRENT_TIMESTAMP as archived_date
    FROM {database}.train_data_out
    WITH DATA;

CREATE TABLE {database}.test_data_archive AS
    SELECT *, CURRENT_TIMESTAMP as archived_date
    FROM {database}.test_data_out
    WITH DATA;
...
AND (
    ...
    OR TableName LIKE '%train%'
    OR TableName LIKE '%test%'
    OR TableName LIKE '%validation%'
)
ORDER BY CreateTimeStamp DESC;

-- Option 6: Collect Statistics after cleanup
/*
COLLECT STATISTICS ON {database}.train_data_archive;
COLLECT STATISTICS ON {database}.test_data_archive;
*/
