-- Complete TD_TrainTestSplit Workflow Template
...
-- 3. TD_TrainTestSplit Execution
DROP TABLE IF EXISTS td_traintestsplit_results;
CREATE MULTISET TABLE td_traintestsplit_results AS (
    SELECT * FROM TD_TrainTestSplit (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_TrainTestSplit parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_traintestsplit_results ORDER BY 1;
