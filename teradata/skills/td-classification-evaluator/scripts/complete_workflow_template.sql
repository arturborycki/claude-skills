-- Complete TD_ClassificationEvaluator Workflow Template
...
-- 3. TD_ClassificationEvaluator Execution
DROP TABLE IF EXISTS td_classificationevaluator_results;
CREATE MULTISET TABLE td_classificationevaluator_results AS (
    SELECT * FROM TD_ClassificationEvaluator (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_ClassificationEvaluator parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_classificationevaluator_results ORDER BY 1;
