-- Complete TD_Ngramsplitter Workflow Template
...
-- 3. TD_Ngramsplitter Execution
DROP TABLE IF EXISTS td_ngramsplitter_results;
CREATE MULTISET TABLE td_ngramsplitter_results AS (
    SELECT * FROM TD_Ngramsplitter (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_Ngramsplitter parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_ngramsplitter_results ORDER BY 1;
