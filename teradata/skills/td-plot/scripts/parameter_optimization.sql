-- =====================================================
-- TD_Plot - Parameter Optimization
-- =====================================================
-- Purpose: Optimize parameters for time series visualization
-- Function: TD_Plot with parameter testing
...
DROP TABLE IF EXISTS {USER_DATABASE}.plot_config1;
CREATE MULTISET TABLE {USER_DATABASE}.plot_config1 AS (
    SELECT * FROM TD_Plot (
        ON {USER_DATABASE}.uaf_plot_input
        USING
        TimeColumn('time_index')
        ValueColumn('series_value')
        -- PlotType: Configure based on your requirements
        -- Resolution: Configure based on your requirements
...
DROP TABLE IF EXISTS {USER_DATABASE}.plot_config2;
CREATE MULTISET TABLE {USER_DATABASE}.plot_config2 AS (
    SELECT * FROM TD_Plot (
        ON {USER_DATABASE}.uaf_plot_input
        USING
        TimeColumn('time_index')
...
DROP TABLE IF EXISTS {USER_DATABASE}.plot_config3;
CREATE MULTISET TABLE {USER_DATABASE}.plot_config3 AS (
    SELECT * FROM TD_Plot (
        ON {USER_DATABASE}.uaf_plot_input
        USING
        TimeColumn('time_index')
...
FROM {USER_DATABASE}.plot_config1
...
SELECT
    ...
    AVG(result_value),
    STDDEV(result_value)
FROM {USER_DATABASE}.plot_config2
...
SELECT
    ...
    AVG(result_value),
    STDDEV(result_value)
FROM {USER_DATABASE}.plot_config3
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config1 WHERE result_value IS NOT NULL) as config1_valid_results,
    (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config2 WHERE result_value IS NOT NULL) as config2_valid_results,
    (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config3 WHERE result_value IS NOT NULL) as config3_valid_results,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config1 WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config2 WHERE result_value IS NOT NULL) AND
             (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config1 WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config3 WHERE result_value IS NOT NULL)
        THEN 'Configuration 1'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config2 WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.plot_config3 WHERE result_value IS NOT NULL)
...
SELECT
    ...
    'Use selected parameters in td_plot_workflow.sql' as next_step;
...
DROP TABLE {USER_DATABASE}.plot_config1;
DROP TABLE {USER_DATABASE}.plot_config2;
DROP TABLE {USER_DATABASE}.plot_config3;
*/
