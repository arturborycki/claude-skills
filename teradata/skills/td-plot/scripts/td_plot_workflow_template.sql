-- Complete TD_PLOT UAF Workflow Template
...
-- 2. TD_PLOT Execution
DROP TABLE IF EXISTS td_plot_results;
CREATE MULTISET TABLE td_plot_results AS (
    SELECT * FROM TD_PLOT (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- PlotType (configure based on your analysis requirements)
        -- Title (configure based on your analysis requirements)
        -- XAxisLabel (configure based on your analysis requirements)
        -- YAxisLabel (configure based on your analysis requirements)
...
FROM td_plot_results;

-- 4. Results Export (if needed)
SELECT * FROM td_plot_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_plot_results;
