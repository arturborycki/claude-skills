-- =====================================================
-- TD_StationarityTest - UAF Performance Monitoring
-- =====================================================
-- Purpose: Monitor and optimize TD_StationarityTest execution
-- Function: TD_StationarityTest performance tracking
...
SELECT
    ...
    'TD_StationarityTest',
    '{USER_DATABASE}',
    'uaf_stationarity_test_input',
    CURRENT_TIMESTAMP,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input),
    'RUNNING';
...
WHERE DatabaseName = '{USER_DATABASE}'
  AND TableName IN ('uaf_stationarity_test_input', 'stationarity_test_results')
ORDER BY TableName;
...
FROM {USER_DATABASE}.uaf_stationarity_test_input
GROUP BY HashAmp()
ORDER BY amp_number;
...
-- Monitor recent TD_StationarityTest executions
SELECT
    QueryID,
    UserName,
    StartTime,
    ...
    NumResultRows
FROM DBC.QryLog
WHERE QueryText LIKE '%TD_StationarityTest%'
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input) as n_input_observations,
    (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results) as n_output_results,
    (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results WHERE result_value IS NOT NULL) as n_valid_results,
    CAST(
        (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results WHERE result_value IS NOT NULL) /
        CAST((SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results) AS FLOAT) * 100
        AS DECIMAL(8,2)
    ) as completeness_pct,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results) AS FLOAT) >= 0.95
        THEN 'Excellent - High completeness (>=95%)'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results) AS FLOAT) >= 0.80
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results) as total_result_rows,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'stationarity_test_results') as total_size_mb,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'stationarity_test_results') / NULLIF((SELECT COUNT(*) FROM {USER_DATABASE}.stationarity_test_results), 0) as avg_mb_per_1000_rows;

-- Identify optimization opportunities
SELECT
    'Optimization Recommendations' as recommendation_type,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input) > 10000
    ...
    CASE
        WHEN (SELECT CurrentPerm / 1024 / 1024
              FROM DBC.TableSize
              WHERE DatabaseName = '{USER_DATABASE}' AND TableName = 'stationarity_test_results') > 1000
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input WHERE series_value IS NULL) as null_values,
    (SELECT COUNT(*)
     FROM {USER_DATABASE}.uaf_stationarity_test_input
     WHERE ABS(series_value - (SELECT AVG(series_value) FROM {USER_DATABASE}.uaf_stationarity_test_input)) >
           3 * (SELECT STDDEV(series_value) FROM {USER_DATABASE}.uaf_stationarity_test_input)) as outliers_3sd,
    (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_stationarity_test_input) as distinct_timestamps,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input) as total_rows,
    CASE
        WHEN (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_stationarity_test_input) =
             (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_stationarity_test_input)
...
WHERE function_name = 'TD_StationarityTest'
...
-- Overall UAF TD_StationarityTest health metrics
SELECT
...
SELECT
    ...
    CASE
        WHEN (SELECT SUM(CurrentPerm) / 1024 / 1024
              FROM DBC.TableSize
              WHERE DatabaseName = '{USER_DATABASE}'
                AND TableName LIKE '%stationarity_test%') > 5000
