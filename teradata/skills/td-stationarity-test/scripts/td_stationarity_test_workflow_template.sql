-- Complete TD_STATIONARITY_TEST UAF Workflow Template
...
-- 2. TD_STATIONARITY_TEST Execution
DROP TABLE IF EXISTS td_stationarity_test_results;
CREATE MULTISET TABLE td_stationarity_test_results AS (
    SELECT * FROM TD_STATIONARITY_TEST (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- TestType (configure based on your analysis requirements)
        -- Lags (configure based on your analysis requirements)
        -- Trend (configure based on your analysis requirements)
        -- ConfidenceLevel (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_STATIONARITY_TEST parameters
    ) as dt
) WITH DATA;
...
FROM td_stationarity_test_results;

-- 4. Results Export (if needed)
SELECT * FROM td_stationarity_test_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_stationarity_test_results;
