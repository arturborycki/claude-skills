-- Complete TD_RegressionEvaluator Workflow Template
...
-- 3. TD_RegressionEvaluator Execution
DROP TABLE IF EXISTS td_regressionevaluator_results;
CREATE MULTISET TABLE td_regressionevaluator_results AS (
    SELECT * FROM TD_RegressionEvaluator (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_RegressionEvaluator parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_regressionevaluator_results ORDER BY 1;
