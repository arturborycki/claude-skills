-- =====================================================
-- TD_RandomForest - Model Training Script
-- =====================================================
-- Purpose: Train random forest classification model
...
DROP TABLE IF EXISTS {database}.rf_model_out;
CREATE MULTISET TABLE {database}.rf_model_out AS (
...
SELECT * FROM {database}.rf_model_out
ORDER BY node_id;
...
SELECT
    'Random Forest Training Complete' as status,
    (SELECT COUNT(*) FROM {database}.rf_train_test_split WHERE train_flag = 1) as training_samples,
    ...
    (SELECT MAX(depth) FROM {database}.rf_model_out) as tree_depth,
