-- =====================================================
-- TD_RandomForestPredict - Prediction Script
-- =====================================================
-- Purpose: Generate predictions using trained random forest model
...
DROP TABLE IF EXISTS {database}.rf_predictions_out;
CREATE MULTISET TABLE {database}.rf_predictions_out AS (
    SELECT * FROM TD_RandomForestPredict (
        ON (SELECT * FROM {database}.rf_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.rf_model_out AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
...
SELECT
    'Random Forest Predictions Complete' as status,
    (SELECT COUNT(*) FROM {database}.rf_predictions_out) as total_predictions,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.rf_predictions_out) as overall_accuracy,
    (SELECT AVG(confidence) FROM {database}.rf_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.rf_predictions_out WHERE confidence < 0.6) as low_confidence_count,
    (SELECT COUNT(*) FROM {database}.rf_predictions_out WHERE {target_column} <> prediction) as misclassification_count;
...
--
-- 2. Prerequisites:
--    - Trained model table (rf_model_out) must exist
