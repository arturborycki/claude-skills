-- Complete TD_DATA_PREPARATION UAF Workflow Template
...
-- 2. TD_DATA_PREPARATION Execution
DROP TABLE IF EXISTS td_data_preparation_results;
CREATE MULTISET TABLE td_data_preparation_results AS (
    SELECT * FROM TD_DATA_PREPARATION (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- ValidationType (configure based on your analysis requirements)
        -- TimeColumn (configure based on your analysis requirements)
        -- ValueColumn (configure based on your analysis requirements)
        -- FillMethod (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_DATA_PREPARATION parameters
    ) as dt
) WITH DATA;
...
FROM td_data_preparation_results;

-- 4. Results Export (if needed)
SELECT * FROM td_data_preparation_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_data_preparation_results;
