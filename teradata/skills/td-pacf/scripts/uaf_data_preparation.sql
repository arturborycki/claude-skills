-- =====================================================
-- TD_PACF - UAF Data Preparation
-- =====================================================
-- Purpose: Prepare time series data for UAF partial auto-correlation
-- Function: TD_PACF
...
DROP TABLE IF EXISTS {USER_DATABASE}.uaf_pacf_input;
CREATE MULTISET TABLE {USER_DATABASE}.uaf_pacf_input AS (
    SELECT
        {TIMESTAMP_COLUMN} as time_index,
...
SELECT
    'UAF TD_PACF Readiness' as status_type,
    COUNT(*) as n_observations,
    CASE
    ...
    CASE
        WHEN COUNT(*) >= 30 THEN 'READY FOR TD_PACF'
