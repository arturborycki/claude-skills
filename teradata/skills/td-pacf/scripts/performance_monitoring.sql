-- =====================================================
-- TD_PACF - UAF Performance Monitoring
-- =====================================================
-- Purpose: Monitor and optimize TD_PACF execution
-- Function: TD_PACF performance tracking
...
SELECT
    ...
    'uaf_pacf_input',
    CURRENT_TIMESTAMP,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input),
    'RUNNING';
...
WHERE DatabaseName = '{USER_DATABASE}'
  AND TableName IN ('uaf_pacf_input', 'pacf_results')
ORDER BY TableName;
...
FROM {USER_DATABASE}.uaf_pacf_input
GROUP BY HashAmp()
ORDER BY amp_number;
...
-- Monitor recent TD_PACF executions
SELECT
    QueryID,
    UserName,
    StartTime,
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input) as n_input_observations,
    (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results) as n_output_results,
    (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results WHERE result_value IS NOT NULL) as n_valid_results,
    CAST(
        (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results WHERE result_value IS NOT NULL) /
        CAST((SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results) AS FLOAT) * 100
        AS DECIMAL(8,2)
    ) as completeness_pct,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results) AS FLOAT) >= 0.95
        THEN 'Excellent - High completeness (>=95%)'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results WHERE result_value IS NOT NULL) /
             CAST((SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results) AS FLOAT) >= 0.80
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results) as total_result_rows,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'pacf_results') as total_size_mb,
    (SELECT SUM(CurrentPerm) / 1024 / 1024
     FROM DBC.TableSize
     WHERE DatabaseName = '{USER_DATABASE}'
       AND TableName = 'pacf_results') / NULLIF((SELECT COUNT(*) FROM {USER_DATABASE}.pacf_results), 0) as avg_mb_per_1000_rows;

-- Identify optimization opportunities
SELECT
    'Optimization Recommendations' as recommendation_type,
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input) > 10000
    ...
    CASE
        WHEN (SELECT CurrentPerm / 1024 / 1024
              FROM DBC.TableSize
              WHERE DatabaseName = '{USER_DATABASE}' AND TableName = 'pacf_results') > 1000
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input WHERE series_value IS NULL) as null_values,
    (SELECT COUNT(*)
     FROM {USER_DATABASE}.uaf_pacf_input
     WHERE ABS(series_value - (SELECT AVG(series_value) FROM {USER_DATABASE}.uaf_pacf_input)) >
           3 * (SELECT STDDEV(series_value) FROM {USER_DATABASE}.uaf_pacf_input)) as outliers_3sd,
    (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_pacf_input) as distinct_timestamps,
    (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input) as total_rows,
    CASE
        WHEN (SELECT COUNT(DISTINCT time_index) FROM {USER_DATABASE}.uaf_pacf_input) =
             (SELECT COUNT(*) FROM {USER_DATABASE}.uaf_pacf_input)
...
-- Overall UAF TD_PACF health metrics
SELECT
