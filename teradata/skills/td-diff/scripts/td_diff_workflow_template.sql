-- Complete TD_DIFF UAF Workflow Template
...
-- 2. TD_DIFF Execution
DROP TABLE IF EXISTS td_diff_results;
CREATE MULTISET TABLE td_diff_results AS (
    SELECT * FROM TD_DIFF (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- DifferenceOrder (configure based on your analysis requirements)
        -- SeasonalDifference (configure based on your analysis requirements)
...
FROM td_diff_results;

-- 4. Results Export (if needed)
SELECT * FROM td_diff_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_diff_results;
