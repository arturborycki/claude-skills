-- =====================================================
-- TD_Diff - UAF Data Preparation
-- =====================================================
-- Purpose: Prepare time series data for UAF time series differencing
-- Function: TD_Diff
...
DROP TABLE IF EXISTS {USER_DATABASE}.uaf_diff_input;
CREATE MULTISET TABLE {USER_DATABASE}.uaf_diff_input AS (
    SELECT
        {TIMESTAMP_COLUMN} as time_index,
...
SELECT
    'UAF TD_Diff Readiness' as status_type,
    COUNT(*) as n_observations,
    CASE
    ...
    CASE
        WHEN COUNT(*) >= 30 THEN 'READY FOR TD_Diff'
