-- Complete TD_DETREND UAF Workflow Template
...
-- 2. TD_DETREND Execution
DROP TABLE IF EXISTS td_detrend_results;
CREATE MULTISET TABLE td_detrend_results AS (
    SELECT * FROM TD_DETREND (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- DetrendType (configure based on your analysis requirements)
        -- PolynomialOrder (configure based on your analysis requirements)
        -- BreakPoints (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_DETREND parameters
    ) as dt
) WITH DATA;
...
SELECT * FROM td_detrend_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_detrend_results;
