-- Complete TD_CHANGE_POINT UAF Workflow Template
...
-- 2. TD_CHANGE_POINT Execution
DROP TABLE IF EXISTS td_change_point_results;
CREATE MULTISET TABLE td_change_point_results AS (
    SELECT * FROM TD_CHANGE_POINT (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- Method (configure based on your analysis requirements)
        -- MinSegmentLength (configure based on your analysis requirements)
        -- Penalty (configure based on your analysis requirements)
        -- MaxChangePoints (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_CHANGE_POINT parameters
    ) as dt
) WITH DATA;
...
FROM td_change_point_results;

-- 4. Results Export (if needed)
SELECT * FROM td_change_point_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_change_point_results;
