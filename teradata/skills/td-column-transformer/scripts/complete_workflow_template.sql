-- Complete TD_ColumnTransformer Workflow Template
...
-- 3. TD_ColumnTransformer Execution
DROP TABLE IF EXISTS td_columntransformer_results;
CREATE MULTISET TABLE td_columntransformer_results AS (
    SELECT * FROM TD_ColumnTransformer (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_ColumnTransformer parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_columntransformer_results ORDER BY 1;
