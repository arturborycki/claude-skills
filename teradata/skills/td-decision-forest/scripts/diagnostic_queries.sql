-- =====================================================
-- Diagnostic Queries - Decision Forest Analysis
-- =====================================================
...
SELECT
    'Decision Forest Diagnostic Summary' as report_type,
    ...
    (SELECT MAX(depth) FROM {database}.df_model_out) as max_depth,
    ...
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.df_predictions_out) as test_accuracy,
    (SELECT AVG(confidence) FROM {database}.df_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.df_predictions_out WHERE confidence < 0.6) as low_confidence_predictions;
