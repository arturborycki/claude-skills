-- =====================================================
-- TD_DecisionTreePredict - Prediction Script
-- =====================================================
-- Purpose: Generate predictions using trained decision tree model
...
FROM {database}.dt_train_test_split
WHERE train_flag = 0;

-- Step 2: Generate predictions on test set
DROP TABLE IF EXISTS {database}.dt_predictions_out;
CREATE MULTISET TABLE {database}.dt_predictions_out AS (
    SELECT * FROM TD_DecisionTreePredict (
        ON (SELECT * FROM {database}.dt_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.dt_model_out AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
...
SELECT
    'Decision Tree Predictions Complete' as status,
    (SELECT COUNT(*) FROM {database}.dt_predictions_out) as total_predictions,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.dt_predictions_out) as overall_accuracy,
    (SELECT AVG(confidence) FROM {database}.dt_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.dt_predictions_out WHERE confidence < 0.6) as low_confidence_count,
    (SELECT COUNT(*) FROM {database}.dt_predictions_out WHERE {target_column} <> prediction) as misclassification_count;
...
--
-- 2. Prerequisites:
--    - Trained model table (dt_model_out) must exist
