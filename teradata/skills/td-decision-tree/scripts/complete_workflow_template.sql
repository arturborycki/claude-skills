-- Complete TD_DecisionTree Workflow Template
...
-- 3. TD_DecisionTree Execution
DROP TABLE IF EXISTS td_decisiontree_results;
CREATE MULTISET TABLE td_decisiontree_results AS (
    SELECT * FROM TD_DecisionTree (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_DecisionTree parameters
    ) as dt
) WITH DATA;

-- 4. Model Prediction
DROP TABLE IF EXISTS td_decisiontree_predictions;
CREATE MULTISET TABLE td_decisiontree_predictions AS (
    SELECT * FROM TD_DecisionTreePredict (
        ON {USER_TABLE}_test as InputTable
        ON td_decisiontree_results as ModelTable DIMENSION
        USING
        IDColumn ('{ID_COLUMN}')
    ) as dt
) WITH DATA;
...
-- 5. Results Analysis
SELECT * FROM td_decisiontree_results ORDER BY 1;
