-- =====================================================
-- TD_DecisionTree - Model Training Script
-- =====================================================
-- Purpose: Train decision tree classification model
...
-- Step 2: Train baseline decision tree model
DROP TABLE IF EXISTS {database}.dt_model_out;
CREATE MULTISET TABLE {database}.dt_model_out AS (
    SELECT * FROM TD_DecisionTree (
        ON (SELECT * FROM {database}.dt_train_test_split WHERE train_flag = 1) AS InputTable
        USING
...
SELECT * FROM {database}.dt_model_out
ORDER BY node_id;
...
SELECT
    'Decision Tree Training Complete' as status,
    (SELECT COUNT(*) FROM {database}.dt_train_test_split WHERE train_flag = 1) as training_samples,
    ...
    (SELECT MAX(depth) FROM {database}.dt_model_out) as tree_depth,
