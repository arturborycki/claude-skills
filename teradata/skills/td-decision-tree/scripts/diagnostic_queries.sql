-- =====================================================
-- Diagnostic Queries - Decision Tree Analysis
-- =====================================================
...
FROM {database}.dt_predictions_out
...
SELECT
    'Decision Tree Diagnostic Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.dt_model_out) as total_nodes,
    (SELECT COUNT(*) FROM {database}.dt_model_out WHERE node_type = 'Leaf') as leaf_nodes,
    (SELECT MAX(depth) FROM {database}.dt_model_out) as max_depth,
    (SELECT COUNT(DISTINCT split_variable) FROM {database}.dt_model_out WHERE node_type = 'Split') as features_used,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.dt_predictions_out) as test_accuracy,
    (SELECT AVG(confidence) FROM {database}.dt_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.dt_predictions_out WHERE confidence < 0.6) as low_confidence_predictions;
