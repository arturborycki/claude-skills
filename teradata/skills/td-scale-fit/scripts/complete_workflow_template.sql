-- Complete TD_ScaleFit Workflow Template
...
-- 3. TD_ScaleFit Execution
DROP TABLE IF EXISTS td_scalefit_results;
CREATE MULTISET TABLE td_scalefit_results AS (
    SELECT * FROM TD_ScaleFit (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_ScaleFit parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_scalefit_results ORDER BY 1;
