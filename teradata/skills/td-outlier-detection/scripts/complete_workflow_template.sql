-- Complete TD_OutlierFit Workflow Template
...
-- 3. TD_OutlierFit Execution
DROP TABLE IF EXISTS td_outlierfit_results;
CREATE MULTISET TABLE td_outlierfit_results AS (
    SELECT * FROM TD_OutlierFit (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_OutlierFit parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_outlierfit_results ORDER BY 1;
