-- Complete TD_MODEL_SELECTION UAF Workflow Template
...
-- 2. TD_MODEL_SELECTION Execution
DROP TABLE IF EXISTS td_model_selection_results;
CREATE MULTISET TABLE td_model_selection_results AS (
    SELECT * FROM TD_MODEL_SELECTION (
        ON uaf_prepared_data
        USING
        -- Function-specific parameters:
        -- SelectionCriteria (configure based on your analysis requirements)
        -- ModelTypes (configure based on your analysis requirements)
        -- CrossValidation (configure based on your analysis requirements)
        -- Add your specific parameters here based on table analysis
        -- Refer to Teradata UAF documentation for TD_MODEL_SELECTION parameters
    ) as dt
) WITH DATA;
...
FROM td_model_selection_results;

-- 4. Results Export (if needed)
SELECT * FROM td_model_selection_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_model_selection_results;
