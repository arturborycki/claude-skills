-- Complete nPath Workflow Template
...
-- 3. nPath Execution
DROP TABLE IF EXISTS npath_results;
CREATE MULTISET TABLE npath_results AS (
    SELECT * FROM nPath (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for nPath parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM npath_results ORDER BY 1;
