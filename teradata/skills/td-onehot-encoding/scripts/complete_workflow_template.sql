-- Complete TD_OneHotEncodingFit Workflow Template
...
-- 3. TD_OneHotEncodingFit Execution
DROP TABLE IF EXISTS td_onehotencodingfit_results;
CREATE MULTISET TABLE td_onehotencodingfit_results AS (
    SELECT * FROM TD_OneHotEncodingFit (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_OneHotEncodingFit parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_onehotencodingfit_results ORDER BY 1;
