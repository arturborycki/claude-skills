-- =====================================================
-- Diagnostic Queries - Logistic Regression Analysis
-- =====================================================
-- Purpose: Analyze coefficients, rules, and performance
...
FROM {database}.logreg_predictions_out
...
SELECT
    'Logistic Regression Diagnostic Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.logreg_model_out) as total_nodes,
    (SELECT COUNT(*) FROM {database}.logreg_model_out WHERE node_type = 'Leaf') as leaf_nodes,
    (SELECT MAX(depth) FROM {database}.logreg_model_out) as max_depth,
    (SELECT COUNT(DISTINCT split_variable) FROM {database}.logreg_model_out WHERE node_type = 'Split') as features_used,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.logreg_predictions_out) as test_accuracy,
    (SELECT AVG(confidence) FROM {database}.logreg_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.logreg_predictions_out WHERE confidence < 0.6) as low_confidence_predictions;
...
--    a. Review coefficients and complexity
