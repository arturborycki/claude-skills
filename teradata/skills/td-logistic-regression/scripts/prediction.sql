-- =====================================================
-- TD_LogisticRegressionPredict - Prediction Script
-- =====================================================
-- Purpose: Generate predictions using trained logistic regression model
...
DROP TABLE IF EXISTS {database}.logreg_predictions_out;
CREATE MULTISET TABLE {database}.logreg_predictions_out AS (
    SELECT * FROM TD_LogisticRegressionPredict (
        ON (SELECT * FROM {database}.logreg_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.logreg_model_out AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
...
SELECT
    'Logistic Regression Predictions Complete' as status,
    (SELECT COUNT(*) FROM {database}.logreg_predictions_out) as total_predictions,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.logreg_predictions_out) as overall_accuracy,
    (SELECT AVG(confidence) FROM {database}.logreg_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.logreg_predictions_out WHERE confidence < 0.6) as low_confidence_count,
    (SELECT COUNT(*) FROM {database}.logreg_predictions_out WHERE {target_column} <> prediction) as misclassification_count;
...
--
-- 2. Prerequisites:
--    - Trained model table (logreg_model_out) must exist
