...
-- =====================================================
-- Purpose: Comprehensive evaluation of logistic regression classifier
...
WITH class_metrics AS (
    SELECT
        ...
        (SELECT COUNT(*)
         FROM {database}.logreg_predictions_out p2
         WHERE p2.prediction = p1.{target_column}
           AND p2.{target_column} <> p2.prediction) as fp
    FROM {database}.logreg_predictions_out p1
...
SELECT
    'Logistic Regression Evaluation Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.logreg_predictions_out) as total_test_samples,
    (SELECT COUNT(DISTINCT {target_column}) FROM {database}.logreg_predictions_out) as num_classes,
    ...
    (SELECT AVG(confidence) FROM {database}.logreg_predictions_out) as avg_prediction_confidence,
    (SELECT MAX(depth) FROM {database}.logreg_model_out) as tree_depth,
