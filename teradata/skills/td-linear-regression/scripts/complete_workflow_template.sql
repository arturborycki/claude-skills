-- Complete TD_LinearRegression Workflow Template
...
-- 3. TD_LinearRegression Execution
DROP TABLE IF EXISTS td_linearregression_results;
CREATE MULTISET TABLE td_linearregression_results AS (
    SELECT * FROM TD_LinearRegression (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_LinearRegression parameters
    ) as dt
) WITH DATA;

-- 4. Model Prediction
DROP TABLE IF EXISTS td_linearregression_predictions;
CREATE MULTISET TABLE td_linearregression_predictions AS (
    SELECT * FROM TD_LinearRegressionPredict (
        ON {USER_TABLE}_test as InputTable
        ON td_linearregression_results as ModelTable DIMENSION
        USING
        IDColumn ('{ID_COLUMN}')
    ) as dt
) WITH DATA;
...
-- 5. Results Analysis
SELECT * FROM td_linearregression_results ORDER BY 1;
