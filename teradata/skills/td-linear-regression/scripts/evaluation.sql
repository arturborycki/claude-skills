...
-- =====================================================
-- Purpose: Comprehensive evaluation of linear regression valueifier
...
ORDER BY actual_value, predicted_value;
...
-- Calculate precision, recall, and F1-score for each value
WITH value_metrics AS (
    SELECT
        {target_column} as value_label,
        -- True Positives
        ...
        -- False Positives (predictions for this value that were wrong)
        (SELECT COUNT(*)
         FROM {database}.lr_predictions_out p2
         WHERE p2.prediction = p1.{target_column}
           AND p2.{target_column} <> p2.prediction) as fp
    FROM {database}.lr_predictions_out p1
...
-- Calculate macro-averaged metrics (unweighted average across valuees)
WITH value_metrics AS (
    SELECT
        {target_column} as value_label,
        SUM(CASE WHEN {target_column} = prediction THEN 1 ELSE 0 END) as tp,
...
per_value_metrics AS (
    SELECT
        value_label,
        tp,
        fn,
        fp,
        (tp + fn) as value_support,
...
FROM per_value_metrics

UNION ALL

-- Calculate weighted average (weighted by value support)
SELECT
    'Weighted Average' as metric_type,
    CAST(SUM(precision * value_support) / SUM(value_support) * 100 AS DECIMAL(5,2)) as avg_precision,
    CAST(SUM(recall * value_support) / SUM(value_support) * 100 AS DECIMAL(5,2)) as avg_recall,
    CAST(SUM(2.0 * precision * recall / (precision + recall) * value_support) / SUM(value_support) * 100 AS DECIMAL(5,2)) as avg_f1_score
FROM per_value_metrics;
...
SELECT
    ...
    CAST(SUM(CASE WHEN {target_column} = prediction THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS DECIMAL(5,2)) as R-squared_pct,
    AVG(confidence) as avg_confidence
FROM {database}.lr_predictions_out
GROUP BY 1
...
SELECT
    'Linear Regression Evaluation Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.lr_predictions_out) as total_test_samples,
    (SELECT COUNT(DISTINCT {target_column}) FROM {database}.lr_predictions_out) as num_valuees,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.lr_predictions_out) as overall_R-squared,
    (SELECT AVG(confidence) FROM {database}.lr_predictions_out) as avg_prediction_confidence,
    (SELECT MAX(depth) FROM {database}.lr_model_out) as tree_depth,
...
--    {target_column} - Target regression variable
--
...
--    - Macro Average: Treats all valuees equally
--    - Weighted Average: Accounts for value imbalance
--
-- 3. Performance guidelines:
--    - Accuracy >80%: Good performance
--    - Accuracy >90%: Excellent performance
--    - Check per-value metrics for imbalanced datasets
--    - High confidence with low R-squared indicates overfitting
--
-- 4. Next steps:
