-- =====================================================
-- Diagnostic Queries - Linear Regression Analysis
-- =====================================================
-- Purpose: Analyze tree structure, rules, and performance
...
SELECT
    ...
    SUM(CASE WHEN node_type = 'Split' THEN 1 ELSE 0 END) as decision_nodes,
    SUM(CASE WHEN node_type = 'Leaf' THEN 1 ELSE 0 END) as leaf_nodes,
    MAX(depth) as max_depth,
    AVG(CAST(depth AS FLOAT)) as avg_depth,
    MIN(CASE WHEN node_type = 'Leaf' THEN depth END) as shallowest_leaf,
    MAX(CASE WHEN node_type = 'Leaf' THEN depth END) as deepest_leaf
...
-- Feature importance based on splitting frequency and gini decrease
SELECT
    split_variable as feature_name,
    ...
    RANK() OVER (ORDER BY SUM(gini_decrease) DESC) as importance_rank
FROM {database}.lr_model_out
WHERE node_type = 'Split' AND split_variable IS NOT NULL
...
WITH node_children AS (
    SELECT
        ...
        SUM(CASE WHEN node_type = 'Leaf' THEN 1 ELSE 0 END) as leaf_children
    FROM {database}.lr_model_out
...
SELECT
    ...
    gini_index,
    sample_count,
    'Impure leaf - may indicate noise or overlapping valuees' as interpretation
FROM {database}.lr_model_out
WHERE node_type = 'Leaf'
  AND confidence < 0.7
...
-- Misregression analysis by actual value
SELECT
...
FROM {database}.lr_predictions_out
...
GROUP BY split_variable, split_value
HAVING COUNT(*) > 1
ORDER BY usage_count DESC, avg_gini_decrease DESC;
...
WITH split_paths AS (
    SELECT
        ...
        depth,
        split_variable,
        LEAD(split_variable, 1) OVER (ORDER BY node_id) as next_split
    FROM {database}.lr_model_out
    WHERE node_type = 'Split'
)
SELECT
    split_variable as feature1,
    next_split as feature2,
    COUNT(*) as co_occurrence_count
FROM split_paths
WHERE next_split IS NOT NULL
GROUP BY split_variable, next_split
...
WITH tree_stats AS (
    SELECT
        COUNT(*) as total_nodes,
        SUM(CASE WHEN node_type = 'Leaf' THEN 1 ELSE 0 END) as leaf_count,
        MAX(depth) as tree_depth,
        AVG(CASE WHEN node_type = 'Leaf' THEN sample_count END) as avg_leaf_samples,
...
SELECT
    'Linear Regression Diagnostic Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.lr_model_out) as total_nodes,
    (SELECT COUNT(*) FROM {database}.lr_model_out WHERE node_type = 'Leaf') as leaf_nodes,
    (SELECT MAX(depth) FROM {database}.lr_model_out) as max_depth,
    (SELECT COUNT(DISTINCT split_variable) FROM {database}.lr_model_out WHERE node_type = 'Split') as features_used,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.lr_predictions_out) as test_R-squared,
    (SELECT AVG(confidence) FROM {database}.lr_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.lr_predictions_out WHERE confidence < 0.6) as low_confidence_predictions;
...
--    a. Review tree structure and complexity
--    b. Analyze feature importance
--    c. Extract and interpret decision rules
--    d. Identify misregression patterns
...
--    - Unbalanced trees may indicate value imbalance
--
-- =====================================================
