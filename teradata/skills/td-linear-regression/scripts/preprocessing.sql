-- =====================================================
-- Data Preprocessing for TD_LinearRegression
-- =====================================================
-- Purpose: Prepare data for linear regression regression
...
SELECT
    ...
    COUNT(DISTINCT {target_column}) as unique_valuees,
...
DROP TABLE IF EXISTS {database}.lr_train_test_split;
CREATE MULTISET TABLE {database}.lr_train_test_split AS (
    SELECT
        *,
        CASE
...
-- Verify value balance in splits
SELECT
    train_flag,
    {target_column} as value_label,
    COUNT(*) as count,
...
ORDER BY train_flag, value_label;
...
/*
DROP TABLE IF EXISTS {database}.lr_encoded_data;
CREATE MULTISET TABLE {database}.lr_encoded_data AS (
    SELECT
        t.*,
    ...
    FROM {database}.lr_train_test_split t
) WITH DATA;
*/
...
/*
DROP TABLE IF EXISTS {database}.lr_preprocessed;
CREATE MULTISET TABLE {database}.lr_preprocessed AS (
    SELECT
        {id_column},
        {target_column},
        train_flag,
...
SELECT
    ...
    COUNT(DISTINCT {target_column}) as num_valuees,
...
--    - Scaling is NOT required for linear regressions
...
--
-- 3. Next steps:
--    - Review value distribution and balance
--    - Proceed to model_training.sql
--    - Use lr_train_test_split table for modeling
--
-- =====================================================
