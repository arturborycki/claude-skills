...
-- =====================================================
-- Purpose: Comprehensive evaluation of support vector machine classifier
...
SELECT
    'SVM Evaluation Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.svm_predictions_out) as total_test_samples,
    (SELECT COUNT(DISTINCT {target_column}) FROM {database}.svm_predictions_out) as num_classes,
    ...
    (SELECT AVG(confidence) FROM {database}.svm_predictions_out) as avg_prediction_confidence,
    (SELECT MAX(depth) FROM {database}.svm_model_out) as tree_depth,
