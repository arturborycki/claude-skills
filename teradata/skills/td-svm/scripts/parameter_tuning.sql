-- =====================================================
-- Parameter Tuning - TD_SVM Optimization
-- =====================================================
-- Purpose: Systematic approach to optimize support vector machine parameters
...
DROP TABLE IF EXISTS {database}.svm_baseline_model;
CREATE MULTISET TABLE {database}.svm_baseline_model AS (
    SELECT * FROM TD_SVM (
...
-- Evaluate baseline
DROP TABLE IF EXISTS {database}.svm_baseline_predictions;
CREATE MULTISET TABLE {database}.svm_baseline_predictions AS (
    SELECT * FROM TD_SVMPredict (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.svm_baseline_model AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
...
SELECT
    ...
    (SELECT MAX(depth) FROM {database}.svm_baseline_model) as tree_depth,
    (SELECT COUNT(*) FROM {database}.svm_baseline_model WHERE sv_type = 'Leaf') as leaf_nodes
FROM {database}.svm_baseline_predictions;
...
DROP TABLE IF EXISTS {database}.svm_model_depth5;
CREATE MULTISET TABLE {database}.svm_model_depth5 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_model_depth15;
CREATE MULTISET TABLE {database}.svm_model_depth15 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_model_depth20;
CREATE MULTISET TABLE {database}.svm_model_depth20 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_model_split10;
CREATE MULTISET TABLE {database}.svm_model_split10 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_model_split50;
CREATE MULTISET TABLE {database}.svm_model_split50 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_model_entropy;
CREATE MULTISET TABLE {database}.svm_model_entropy AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
DROP TABLE IF EXISTS {database}.svm_entropy_predictions;
CREATE MULTISET TABLE {database}.svm_entropy_predictions AS (
    SELECT * FROM TD_SVMPredict (
        ON (SELECT * FROM {database}.svm_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.svm_model_entropy AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
        Accumulate ('{id_column}', '{target_column}')
    ) as dt
...
DROP TABLE IF EXISTS {database}.svm_cv_splits;
CREATE MULTISET TABLE {database}.svm_cv_splits AS (
    SELECT
        *,
...
DROP TABLE IF EXISTS {database}.svm_cv_model_fold1;
CREATE MULTISET TABLE {database}.svm_cv_model_fold1 AS (
    SELECT * FROM TD_SVM (
        ON (SELECT * FROM {database}.svm_cv_splits WHERE fold_id <> 1) AS InputTable
        USING
        ResponseColumn ('{target_column}')
        InputColumns ({feature_columns})
        IDColumn ('{id_column}')
...
DROP TABLE IF EXISTS {database}.svm_cv_predictions_fold1;
CREATE MULTISET TABLE {database}.svm_cv_predictions_fold1 AS (
    SELECT
        1 as fold_id,
        dt.*
    FROM TD_SVMPredict (
        ON (SELECT * FROM {database}.svm_cv_splits WHERE fold_id = 1) AS InputTable
        ON {database}.svm_cv_model_fold1 AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
        Accumulate ('{id_column}', '{target_column}')
    ) as dt
...
DROP TABLE IF EXISTS {database}.svm_model_comparison;
CREATE MULTISET TABLE {database}.svm_model_comparison AS (
    -- Baseline model
    SELECT
    ...
    SELECT
        ...
        CAST(SUM(CASE WHEN {target_column} = prediction THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS DECIMAL(5,2)) as accuracy,
        AVG(confidence) as avg_confidence,
        (SELECT MAX(depth) FROM {database}.svm_model_entropy) as tree_depth,
        (SELECT COUNT(*) FROM {database}.svm_model_entropy WHERE sv_type = 'Leaf') as leaf_nodes
    FROM {database}.svm_entropy_predictions
...
FROM {database}.svm_model_comparison
...
DROP TABLE IF EXISTS {database}.svm_final_optimized_model;
CREATE MULTISET TABLE {database}.svm_final_optimized_model AS (
    SELECT * FROM TD_SVM (
        ON {database}.svm_train_test_split AS InputTable
        USING
        ResponseColumn ('{target_column}')
...
FROM {database}.svm_final_optimized_model;
