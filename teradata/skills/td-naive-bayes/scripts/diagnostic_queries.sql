-- =====================================================
-- Diagnostic Queries - Naive Bayes Analysis
-- =====================================================
-- Purpose: Analyze probability tables, rules, and performance
...
-- Feature importance based on splitting frequency and probability decrease
SELECT
    feature as feature_name,
    COUNT(*) as split_count,
    AVG(probability_decrease) as avg_probability_decrease,
    SUM(probability_decrease) as total_probability_decrease,
    AVG(sample_count) as avg_samples_at_split,
    MIN(depth) as first_split_depth,
    RANK() OVER (ORDER BY SUM(probability_decrease) DESC) as importance_rank
FROM {database}.nb_model_out
WHERE node_type = 'Split' AND feature IS NOT NULL
GROUP BY feature
ORDER BY total_probability_decrease DESC;
...
ORDER BY probability_decrease DESC
LIMIT 10;
...
GROUP BY feature, split_value
HAVING COUNT(*) > 1
ORDER BY usage_count DESC, avg_probability_decrease DESC;
...
WITH split_paths AS (
    SELECT
        ...
        depth,
        feature,
        LEAD(feature, 1) OVER (ORDER BY node_id) as next_split
    FROM {database}.nb_model_out
    WHERE node_type = 'Split'
)
SELECT
...
SELECT
    'Naive Bayes Diagnostic Summary' as report_type,
    (SELECT COUNT(*) FROM {database}.nb_model_out) as total_nodes,
    (SELECT COUNT(*) FROM {database}.nb_model_out WHERE node_type = 'Leaf') as leaf_nodes,
    (SELECT MAX(depth) FROM {database}.nb_model_out) as max_depth,
    (SELECT COUNT(DISTINCT feature) FROM {database}.nb_model_out WHERE node_type = 'Split') as features_used,
    (SELECT CAST(AVG(CASE WHEN {target_column} = prediction THEN 100.0 ELSE 0.0 END) AS DECIMAL(5,2))
     FROM {database}.nb_predictions_out) as test_accuracy,
    (SELECT AVG(confidence) FROM {database}.nb_predictions_out) as avg_confidence,
    (SELECT COUNT(*) FROM {database}.nb_predictions_out WHERE confidence < 0.6) as low_confidence_predictions;
...
--    a. Review probability tables and complexity
