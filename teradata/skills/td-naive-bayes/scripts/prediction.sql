-- =====================================================
-- TD_NaiveBayesPredict - Prediction Script
-- =====================================================
-- Purpose: Generate predictions using trained naive bayes model
...
DROP TABLE IF EXISTS {database}.nb_predictions_out;
CREATE MULTISET TABLE {database}.nb_predictions_out AS (
    SELECT * FROM TD_NaiveBayesPredict (
        ON (SELECT * FROM {database}.nb_train_test_split WHERE train_flag = 0) AS InputTable
        ON {database}.nb_model_out AS ModelTable DIMENSION
        USING
        IDColumn ('{id_column}')
...
SELECT
    'Naive Bayes Predictions Complete' as status,
    (SELECT COUNT(*) FROM {database}.nb_predictions_out) as total_predictions,
    ...
    (SELECT COUNT(*) FROM {database}.nb_predictions_out WHERE confidence < 0.6) as low_confidence_count,
    (SELECT COUNT(*) FROM {database}.nb_predictions_out WHERE {target_column} <> prediction) as misclassification_count;
...
--
-- 2. Prerequisites:
--    - Trained model table (nb_model_out) must exist
