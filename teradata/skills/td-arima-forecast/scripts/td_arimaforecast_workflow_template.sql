-- Complete TD_ARIMAFORECAST UAF Workflow Template
...
-- 2. TD_ARIMAFORECAST Execution
DROP TABLE IF EXISTS td_arimaforecast_results;
CREATE MULTISET TABLE td_arimaforecast_results AS (
    SELECT * FROM TD_ARIMAFORECAST (
        ...
        -- IncludeFit (configure based on your analysis requirements)
...
FROM td_arimaforecast_results;

-- 4. Results Export (if needed)
SELECT * FROM td_arimaforecast_results
ORDER BY time_index;

-- 6. Cleanup (optional)
-- DROP TABLE uaf_prepared_data;
-- DROP TABLE td_arimaforecast_results;
