...
-- ============================================================================
-- PIPELINE STAGE 2: Primary Analysis with TD_ARIMAFORECAST
-- ============================================================================

-- Execute primary UAF function
DROP TABLE IF EXISTS {USER_DATABASE}.pipeline_arimaforecast_results;
CREATE MULTISET TABLE {USER_DATABASE}.pipeline_arimaforecast_results AS (
...
FROM {USER_DATABASE}.pipeline_arimaforecast_results;
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results) as primary_results,
...
SELECT
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results) * 0.95
        THEN 'Excellent'
        WHEN (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results WHERE result_value IS NOT NULL) >=
             (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results) * 0.80
        THEN 'Good'
        ELSE 'Needs Review'
...
CREATE MULTISET TABLE {USER_DATABASE}.pipeline_visualization AS (
    ...
    LEFT JOIN {USER_DATABASE}.pipeline_arimaforecast_results r
...
SELECT
    ...
    (SELECT COUNT(*) FROM {USER_DATABASE}.pipeline_arimaforecast_results) as primary_output_rows,
    (SELECT COUNT(DISTINCT result_type) FROM {USER_DATABASE}.pipeline_arimaforecast_results) as result_types,
    CAST((SELECT AVG(series_value) FROM {USER_DATABASE}.uaf_pipeline_base) AS DECIMAL(12,4)) as input_avg,
    CAST((SELECT AVG(result_value) FROM {USER_DATABASE}.pipeline_arimaforecast_results) AS DECIMAL(12,4)) as output_avg;
...
SELECT
    ...
    'TD_ARIMAFORECAST + Complementary Analyses' as pipeline_components,
...
-- - pipeline_arimaforecast_results (primary analysis)
...
□ Primary analysis (TD_ARIMAFORECAST) executed
