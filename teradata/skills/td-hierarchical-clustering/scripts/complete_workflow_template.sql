-- Complete TD_HierarchicalClustering Workflow Template
...
-- 3. TD_HierarchicalClustering Execution
DROP TABLE IF EXISTS td_hierarchicalclustering_results;
CREATE MULTISET TABLE td_hierarchicalclustering_results AS (
    SELECT * FROM TD_HierarchicalClustering (
        ON {USER_DATABASE}.{USER_TABLE} as InputTable
        USING
        -- Add function-specific parameters here
        -- Refer to Teradata documentation for TD_HierarchicalClustering parameters
    ) as dt
) WITH DATA;

-- 5. Results Analysis
SELECT * FROM td_hierarchicalclustering_results ORDER BY 1;
